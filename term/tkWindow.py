...
class TkWindow:
    def __init__(self):
        ...
        # 이 3개가 조회 탭
        self.setCategoriFrame(self.window)
        self.setMainFrame(self.window)  # selectViewMode Notebook 추가 예정
        self.setPageFrame(self.window)
    ...
    def setMainFrame(self, master):
        self.mainFrame = Frame(master, width=width, height=400)
        self.mainFrame.pack(side="top", expand=True, fill="both", padx=5)

        # scrollbar 추가를 위해서 canvas 사용
        self.mainScrollbar = Scrollbar(self.mainFrame, orient="vertical")
        self.mainScrollbar.pack(side="right", fill="y")
        self.mainCanvas = Canvas(self.mainFrame, width=width, height=400,
        ...
        for i in range(self.numOfPage):
            label = ListViewLabel(self.mainCanvas, font12, width=width, height=400, x=0,y=i*400)
